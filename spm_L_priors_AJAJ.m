function [pE,pC] = spm_L_priors_AJAJ(dipfit,pE,pC)
...
for i = 1:numel(model)
    ...
    switch upper(model(i).source)
        ...
        case{'CMM_NMDA','CMM_NMDA_AJ'}
            % there are different J corresponds to each region
            %--------------------------------------------------------------
%              pE.J{end + 1} = sparse(1,2,1,1,16);              % 12 states
              pE.J{end + 1} = sparse(1:m, 2, ones(m, 1), m, 16);
%             pC.J{end + 1} = sparse(1,[3,4],1/32,1,16);
              pC.J{end + 1} = sparse(repmat((1:m)', 1, 2), repmat([3, 4], m, 1), 1/32, m, 16); 
        case{'MFM'}
            %--------------------------------------------------------------
