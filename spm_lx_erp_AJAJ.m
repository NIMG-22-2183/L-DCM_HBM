function [L] = spm_lx_erp_AJAJ(P,dipfit)
...
% This code supports differnt J for regions. Orginal code only consider one
% set of J for all regions. 
%--------------------------------------------------------------------------
L     = spm_erp_L(P,dipfit);               % lead field per source
SP = zeros(4,16*size(L,2));
for i = 1:size(L,2)
    SP(i,:) = kron(P.J(i,:),L(i,:));
end
L = sparse(SP);
