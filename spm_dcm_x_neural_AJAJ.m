...
if isstruct(model)
    ...
    P.A{1} = 1;
    for i = 1:numel(model)
        x{i} = spm_dcm_x_neural_AJAJ(P,model(i).source);
    end
...
switch lower(model)
  ...
  % Canonical mass model with NMDA (nonlinear in states)
    ...
    case{'cmm_nmda_aj'}
        ...
        % inital states and model
        %------------------------------------------------------------------
        x  = spm_x_cmm_NMDA_AJAJ(P);
        f  = 'spm_fx_cmm_NMDA_AJAJ';
